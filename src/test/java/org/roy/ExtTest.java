...
public class ExtTest {
	...
	public void testLocalDateTransferdate() {
		...
		Date from1 = Date.from(LocalDateTime.now().minusDays(1L).atZone(ZoneId.systemDefault()).toInstant());
		System.out.println(from1);

		String s = new BigDecimal(String.valueOf(new Random().nextInt(100))).multiply(BigDecimal.valueOf(100)).setScale(2, BigDecimal.ROUND_HALF_UP).toString() + "%";
	...
	public void testXXXX() {
		String s = new BigDecimal(String.valueOf(new Random().nextInt(10000))).divide(BigDecimal.valueOf(100)).setScale(2, BigDecimal.ROUND_HALF_UP).toString() + "%";
		System.out.println(s);
	}
	...
	public void testRemoveDuplicate() {
		...
		List<Employee> employees = data.stream().collect(Collectors.collectingAndThen(
