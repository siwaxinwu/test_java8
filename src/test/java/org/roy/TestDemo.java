...
public class TestDemo {
  ...
  /**
   * 这个方法有两个参数，Key 和一个根据 Key 来产生 Value 的 Function；然后返回一个 Value。 这个方法会检查 Map 中的 Key，如果发现 Key
   * 不存在或者对应的值是 null，则调用 Function 来产生一个值，然后将其放入 Map， 最后返回这个值；否则的话返回 Map 已经存在的值。
   *
   * <p>V getOrDefault(Object, V) 这个方法同样检查 Map 中的 Key，如果发现 Key 不存在或者对应的 value 值是
   * null，则返回第二个参数即默认值。要注意，这个默认值不会放入 Map
   *
   * <p>V putIfAbsent(K, V) 这个方法的逻辑完全不同，注意它不是一个 get() 方法，而是 put() 方法的变种！这个方法的逻辑是，如果 Key 不存在或者对应的值是
   * null， 则将 Value 设置进去，
   */
  @Test
  ...
  * */
  ...
  public void test4() {
    ...
    System.out.println("----------------------------------------------");
    BinaryOperator<Integer> operator2 =
  ...
  public void test8() {
    ...
    Function<String, Employee> function =
        (str) -> {
          return new Employee(str, 24, 23.5);
        };
  ...
  public void test10() {
    List<String> result =
        TestDemo.createData().stream().map(Employee::getName).collect(Collectors.toList());
    System.out.println(result);
    System.out.println("-----------------------------------");
  ...
  public void test11() {
    boolean b =
        TestDemo.createEnumData().stream()
            .allMatch(
                (ele) -> {
  ...
  public void test12() {
    ...
    System.out.println("-------------------------------------------------");
    Double aDouble =
        TestDemo.createEnumData().stream().map(Employee::getSalary).reduce(Double::sum).get();
  ...
  public void test13() {
    ...
    DoubleSummaryStatistics result1 =
        TestDemo.createEnumData().stream()
